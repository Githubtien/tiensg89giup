...
def Lay_somade_tu30bsx(cnts_30bsx_toadocu,paper):
    gray = cv2.cvtColor(paper, cv2.COLOR_BGR2GRAY) 
    str_somd=''
    means=[]
    for c in cnts_30bsx_toadocu: #30 pt
...
def Lay_sobd_tu60bsx(cnts_60bsx_toadocu,paper):
    gray = cv2.cvtColor(paper, cv2.COLOR_BGR2GRAY) 
    str_sobd=''
    means=[]
    for c in cnts_60bsx_toadocu: #60 pt
        x,y,w,h = cv2.boundingRect(c)
        anh_hcnbao_cnt=gray[y:y+h,x:x+w]
        anh_hcnbao_cnt=Xen_xquanh_anh(anh_hcnbao_cnt,beday=2)
        #brow_img(anh_hcnbao_cnt,'XXX')
...
def cham_ptn_008_40(image,dic_dap_an):
    ...
    #cv2.drawContours(paper, cnts[:1], -1, (0, 0, 255), 5)
    #brow_img(paper,'xxxxxx')
    ...
    for c in cnts:
        ...
        if (0.8 < float(w/h) < 1.2) and len(approx) != 4:
            cnt_bubs.append(c)
    ...
    cnt_bubs=cnt_bubs[:250]
    #cv2.drawContours(anh_phan_tn, cnt_bubs, -1, (0, 0,255), 1)
    #brow_img(anh_phan_tn,'250bubs')

    cnt_bubs = sorted(cnt_bubs, key=get_x_ver0)

    # lay cac cnts made sx lai roi gui ham Lay_somade_tu30bsx rut ra made
    cnt_bubs_md=cnt_bubs[:30]
    cnts_30bsx=[]
    cnts_30bsx_toadocu=[]
    for i in range(3):
        c_cot_10bubs=cnt_bubs_md[(i%3)*10:(i%3)*10+10]
        c_cot_10bubs=sorted(c_cot_10bubs,key=get_y_ver1)
        cnts_30bsx = cnts_30bsx + c_cot_10bubs
    for c in cnts_30bsx:
    ...
    cnt_bubs_bd=cnt_bubs[30:90]
    #cv2.drawContours(anh_phan_tn, cnt_bubs_bd, -1, (255,0, 0), 2)
    #brow_img(anh_phan_tn,'60b bd')
    cnts_60bsx=[]
    cnts_60bsx_toadocu=[]
    for i in range(6):
        c_cot_10bubs=cnt_bubs_bd[(i%6)*10:(i%6)*10+10]
        c_cot_10bubs=sorted(c_cot_10bubs,key=get_y_ver1)
        cnts_60bsx = cnts_60bsx + c_cot_10bubs
    for c in cnts_60bsx:
    ...
    All_cnts_bub_in_paper=cnt_bubs_tn_sx
