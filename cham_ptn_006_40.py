...
def cham_ptn_006_40(image,dic_dap_an):
    ...
    cnt_bubs=cnts[:160] # cac bubs co dt lon nen lay 160 bub
    #for c in cnts:
    #    x,y,w,h = cv2.boundingRect(c)
    #    if (0.8 < float(w/h) < 1.2):
    #        cnt_bubs.append(c)

    #cv2.drawContours(anh_phan_tn, cnts[:160], -1, (0, 0,255), 1)
        ...
        if len(means)==4 :
            ...
            if answer_choices[min_arg] == dic_dap_an[cau]:
                ...
                #	brow_img(paper,'paper'))
                    ...
                    idx = answer_choices.index(kitu)
                    xb,yb,wb,hb = cv2.boundingRect(All_cnts_bub_in_paper[cau*4 + idx])
                    cv2.circle(paper,(xb+round(wb/2),yb+round(hb/2)),round(wb/2),(0,0,255),2) # RED
                #	brow_img(paper,'paper')
            means = []
    #ket_qua_thi='pppppp'
    if len(questions_answer)>0:
        ...
        cv2.putText(paper, text1, toado1, 0, fontScale, color1, thickness, cv2.LINE_AA)
...
#image=cv2.imread('ptn_006_40.jpg')
#dic_dap_an = Tao_dicdapan_random(40)

#paper = cham_ptn_006_40(image,dic_dap_an)
#brow_img(paper,'KQCC:')
